...
# Get Dataset Here
def prepare_dataset(
    data_dir=None, batch_size=1, test_batch=1, datathread=4, logger=None
):
...
def depth_scale_shift_normalization(depth):
    ...
    min_value = torch.from_numpy(np.percentile(a=depth_, q=2, axis=1)).to(depth)[
        ..., None, None, None
    ]
    max_value = torch.from_numpy(np.percentile(a=depth_, q=98, axis=1)).to(depth)[
        ..., None, None, None
    ]

    normalized_depth = ((depth - min_value) / (max_value - min_value + 1e-5) - 0.5) * 2
    normalized_depth = torch.clip(normalized_depth, -1.0, 1.0)
...
def resize_max_res_tensor(input_tensor, mode, recom_resolution=768):
    ...
    if mode == "normal":
        resized_input_tensor = F.interpolate(
            input_tensor, scale_factor=downscale_factor, mode="nearest"
        )
    else:
        resized_input_tensor = F.interpolate(
            input_tensor,
