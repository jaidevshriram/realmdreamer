...
def _render_trajectory_video(
    ...
    image_format: Literal["jpeg", "png", "npy"] = "jpeg",
    ...
    with ExitStack() as stack:
        ...
        max_depth, min_depth = 0.0, 10**6

        if 'depth' in rendered_output_names:

            with progress:
                for camera_idx in progress.track(range(cameras.size), description="Computing Depth Range"):
                    aabb_box = None
                    ...
                    if 'depth' not in outputs:
                        ...
                        CONSOLE.print(f"Could not find 'depth' in the model outputs", justify="center")
                        CONSOLE.print(
                            f"Please set --rendered_output_name to one of: {outputs.keys()}", justify="center"
                        )
                        sys.exit(1)

                    output_image = outputs['depth']

                    max_depth = max(max_depth, output_image.max())
                    min_depth = min(min_depth, output_image.min())

        print(f"min_depth: {min_depth}, max_depth: {max_depth}")

        if colormap_options.specified_normalize:
            colormap_options.colormap_min = min_depth
            colormap_options.colormap_max = max_depth
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                for rendered_output_name in rendered_output_names:
                    ...
                    if rendered_output_names in ['depth']:
                        output_image = output_image ** 3
                        print("cubing depth")

                    np.save(output_image_dir / f"raw_{camera_idx}.npy", output_image.cpu().numpy())
