...
from ..utils.image_util import chw2hwc, colorize_depth_maps, resize_max_res
...
class DepthNormalEstimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # Resize image
        if processing_res > 0:
            input_image = resize_max_res(input_image, max_edge_resolution=processing_res)
        ...
        if ensemble_size > 1:
            depth_pred, pred_uncert = ensemble_depths(depth_preds, **(ensemble_kwargs or {}))
        ...
        # Colorize
        depth_colored = colorize_depth_maps(depth_pred, 0, 1, cmap=color_map).squeeze()  # [3, H, W], value in (0, 1)
    ...
    def __encode_img_embed(self, rgb):
        ...
        """
        clip_image_mean = torch.as_tensor(self.feature_extractor.image_mean)[:, None, None].to(
            device=self.device, dtype=self.dtype
        )
        clip_image_std = torch.as_tensor(self.feature_extractor.image_std)[:, None, None].to(
            device=self.device, dtype=self.dtype
        )
        ...
        img_in_proc = TF.resize(
            ...
            (
                ...
                self.feature_extractor.crop_size["width"],
            ),
    ...
    def single_infer(
        ...
        batch_img_embed = self.img_embed.repeat((rgb_latent.shape[0], 1, 1))  # [B, 1, 768]

        # hybrid switcher
        geo_class = torch.tensor([[0.0, 1.0], [1, 0]], device=device, dtype=self.dtype)
        geo_embedding = torch.cat([torch.sin(geo_class), torch.cos(geo_class)], dim=-1)

        if domain == "indoor":
            domain_class = torch.tensor([[1.0, 0.0, 0]], device=device, dtype=self.dtype).repeat(2, 1)
        elif domain == "outdoor":
            domain_class = torch.tensor([[0.0, 1.0, 0]], device=device, dtype=self.dtype).repeat(2, 1)
        elif domain == "object":
            domain_class = torch.tensor([[0.0, 0.0, 1]], device=device, dtype=self.dtype).repeat(2, 1)
        ...
        for i, t in iterable:
            ...
            noise_pred = self.unet(
                unet_input,
                t.repeat(2),
                encoder_hidden_states=batch_img_embed,
                class_labels=class_embedding,
            ).sample  # [B, 4, h, w]

            # compute the previous noisy sample x_t -> x_t-1
            predicted_dict = self.scheduler.step(noise_pred, t, geo_latent, return_dict=True)
            geo_latent = predicted_dict["prev_sample"]
            one_step_pred = predicted_dict["pred_original_sample"]

            if single_step:
                geo_latent = one_step_pred
                break
