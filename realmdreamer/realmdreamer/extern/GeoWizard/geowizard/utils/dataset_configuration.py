...
# Get Dataset Here
def prepare_dataset(data_dir=None, batch_size=1, test_batch=1, datathread=4, logger=None):
...
def resize_max_res_tensor(input_tensor, mode, recom_resolution=768):
    ...
    if mode == "normal":
        resized_input_tensor = F.interpolate(input_tensor, scale_factor=downscale_factor, mode="nearest")
    else:
