...
if __name__ == "__main__":
    ...
    # declare a pipeline
    pipe = DepthNormalEstimationPipeline.from_pretrained(
    ...
    with torch.no_grad():
        ...
        for test_file in tqdm(test_files, desc="Estimating Depth & Normal", leave=True):
            ...
            normal_npy_save_path = os.path.join(
                output_dir_normal_npy, f"{pred_name_base}.npy"
            )
            if os.path.exists(normal_npy_save_path):
                logging.warning(
                    f"Existing file: '{normal_npy_save_path}' will be overwritten"
                )
            ...
            normal_colored_save_path = os.path.join(
                output_dir_normal_color, f"{pred_name_base}_colored.png"
            )
