...
def kitti_colormap(disparity, maxval=-1):
    ...
    colored_disp[:, :, 2] = (
        w * colormap[index][:, :, 0] + (1.0 - w) * colormap[index + 1][:, :, 0]
    )
    colored_disp[:, :, 1] = (
        w * colormap[index][:, :, 1] + (1.0 - w) * colormap[index + 1][:, :, 1]
    )
    colored_disp[:, :, 0] = (
        w * colormap[index][:, :, 2] + (1.0 - w) * colormap[index + 1][:, :, 2]
    )
...
def read_16bit_gt(path):
    ...
    """
    gt = cv2.imread(path, -1).astype(np.float32) / 256.0
    return gt
