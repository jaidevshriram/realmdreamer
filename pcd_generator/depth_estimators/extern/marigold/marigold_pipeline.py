...
class MarigoldPipeline(DiffusionPipeline):
    ...
    def encode_depths(
        ...
        if upper_quantile != lower_quantile:
            ...
            depth_normalized = depths / lower_quantile
            depth_normalized = (depths - 0.5) * 2.0

        # Repeat depth on 3 channels
        depth_normalized = depth_normalized.repeat(1, 3, 1, 1)

        h = self.vae.encoder(depth_normalized.to(self.dtype))
        moments = self.vae.quant_conv(h)
        mean, logvar = torch.chunk(moments, 2, dim=1)

        # scale latent
        latents = mean * self.depth_latent_scale_factor
    ...
    def inpaint(
        ...
        # Encode image
        rgb_latent = self.encode_rgb(rgb_in.to(self.dtype))
        depth_in_latent = self.encode_depths(depth_in.to(self.dtype))

        # Initial depth map (noise)
        depth_latent = torch.randn(
            rgb_latent.shape, device=self.device, dtype=self.dtype
        )  # [B, 4, h, w]

        mask_latent_shape = F.interpolate(
            mask.float(), (rgb_latent.shape[-2:]), mode="nearest"
        ).to(self.dtype)
        ...
        for i, t in iterable:
            ...
            # Overwide the depth latent
            noise = torch.randn_like(depth_latent)
            depth_in_latent_t = self.scheduler.add_noise(depth_in_latent, noise, t)

            depth_latent = (
                mask_latent_shape * depth_latent
                + (1 - mask_latent_shape) * depth_in_latent_t
            )

        torch.cuda.empty_cache()
        depth = self.decode_depth(depth_latent)

        depth_pred = (depth - depth.min()) / (depth.max() - depth.min())
        depth_pred = depth_pred.cpu().numpy().astype(np.float32)
