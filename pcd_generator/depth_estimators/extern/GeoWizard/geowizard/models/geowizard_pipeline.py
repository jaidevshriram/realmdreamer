...
class DepthNormalEstimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if match_input_res:
            ...
            normal_pred = cv2.resize(
                chw2hwc(normal_pred), input_size, interpolation=cv2.INTER_NEAREST
            )
    ...
    def __encode_img_embed(self, rgb):
        ...
        """
        clip_image_mean = torch.as_tensor(self.feature_extractor.image_mean)[
            :, None, None
        ].to(device=self.device, dtype=self.dtype)
        clip_image_std = torch.as_tensor(self.feature_extractor.image_std)[
            :, None, None
        ].to(device=self.device, dtype=self.dtype)
        ...
        img_in_proc = ((img_in_proc.float() - clip_image_mean) / clip_image_std).to(
            self.dtype
        )
        img_embed = (
            self.image_encoder(img_in_proc).image_embeds.unsqueeze(1).to(self.dtype)
        )
    ...
    def single_infer(
        ...
        if domain == "indoor":
            domain_class = torch.tensor(
                [[1.0, 0.0, 0]], device=device, dtype=self.dtype
            ).repeat(2, 1)
        elif domain == "outdoor":
            domain_class = torch.tensor(
                [[0.0, 1.0, 0]], device=device, dtype=self.dtype
            ).repeat(2, 1)
        elif domain == "object":
            domain_class = torch.tensor(
                [[0.0, 0.0, 1]], device=device, dtype=self.dtype
            ).repeat(2, 1)
        domain_embedding = torch.cat(
            [torch.sin(domain_class), torch.cos(domain_class)], dim=-1
        )
