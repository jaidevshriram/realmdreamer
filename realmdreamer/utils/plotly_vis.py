    ...
    **kwargs,
    ...
    for subplot_idx in range(len(subplots)):
        ...
        if viewpoints_eye_at_up_world is not None:
            ...
            min_range = min(x_range[0], y_range[0], z_range[0])
            max_range = max(x_range[1], y_range[1], z_range[1])
            range_combined = (min_range, max_range)

            eye_x = _scale_camera_to_bounds(eye_x, range_combined, True)
            eye_y = _scale_camera_to_bounds(eye_y, range_combined, True)
            eye_z = _scale_camera_to_bounds(eye_z, range_combined, True)

            at_x = _scale_camera_to_bounds(at_x, range_combined, True)
            at_y = _scale_camera_to_bounds(at_y, range_combined, True)
            at_z = _scale_camera_to_bounds(at_z, range_combined, True)

            up_x = _scale_camera_to_bounds(up_x, range_combined, False)
            up_y = _scale_camera_to_bounds(up_y, range_combined, False)
            up_z = _scale_camera_to_bounds(up_z, range_combined, False)
        ...
        # update the aspect ratio of the axes
        xaxis["range"] = combined_range
        yaxis["range"] = combined_range
        zaxis["range"] = combined_range
