...
def ensemble_depths(
    ...
    # init guess
    _min = np.min(
        input_images.reshape((n_img, -1)).cpu().numpy(), axis=1
    )  # get the min value of each possible depth
    _max = np.max(
        input_images.reshape((n_img, -1)).cpu().numpy(), axis=1
    )  # get the max value of each possible depth
    ...
    s = torch.from_numpy(s).to(dtype=dtype).to(device)
    t = torch.from_numpy(t).to(dtype=dtype).to(device)
    transformed_arrays = original_input * s.view(-1, 1, 1) + t.view(
        -1, 1, 1
    )  # [10,H,W]
