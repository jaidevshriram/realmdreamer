...
@dataclass
class DummyDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: DummyDataParser)
...
@dataclass
class DummyDataParser(DataParser):
    """Nerfstudio DatasetParser"""

    config: DummyDataParserConfig
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        assert self.config.data.suffix == ".json", "Pass a json file as --data"
        ...
        for frame in meta["frames"]:
            ...
            if not fname.exists():
                print("Skipping", fname, "because it does not exist.")
        ...
        if f"{split}_filenames" in meta:
            ...
            num_train_images = math.ceil(num_images * 1)
