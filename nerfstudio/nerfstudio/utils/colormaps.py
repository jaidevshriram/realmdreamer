...
class ColormapOptions:
    ...
    specified_normalize: Optional[bool] = False
    """ Whether to use the specified normalize value """
...
def apply_colormap(
    ...
    if image.shape[-1] == 1 and torch.is_floating_point(image):
        ...
        if colormap_options.specified_normalize:
            ...
            output = (output - colormap_options.colormap_min) / (colormap_options.colormap_max - colormap_options.colormap_min + eps)

            # print("Range", torch.min(output), torch.max(output))

            # fig = plt.figure()
            # plt.imshow(output[:, :, 0].cpu().numpy(), cmap='turbo', vmin=0, vmax=1)
            # plt.savefig('/mnt/data/nerfstudio/test.png')

            output = torch.clip(output, 0, 1)
            if colormap_options.invert:
                output = 1 - output

            # print("Range after clipping", torch.min(output), torch.max(output))
...
def apply_float_colormap(image: Float[Tensor, "*bs 1"], colormap: Colormaps = "viridis") -> Float[Tensor, "*bs rgb=3"]:
    ...
    # print("Range", torch.min(image), torch.max(image))

    image = torch.nan_to_num(image, 0)

    # print("Range after nan_to_num", torch.min(image), torch.max(image))
